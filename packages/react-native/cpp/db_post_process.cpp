...
float PolygonScoreAcc(std::vector<cv::Point> contour, cv::Mat pred) {
  ...
  int xmin = clamp(int(std::floor(*(std::min_element(box_x.begin(),    // NOLINT
                   ...
                   0,                                                  // NOLINT
                   width - 1);
  int xmax = clamp(int(std::ceil(*(std::max_element(box_x.begin(), box_x.end())))),  // NOLINT
                   0,                                                                // NOLINT
                   width - 1);
  int ymin = clamp(int(std::floor(*(std::min_element(box_y.begin(),    // NOLINT
                   ...
                   0,                                                  // NOLINT
                   height - 1);
  int ymax = clamp(int(std::ceil(*(std::max_element(box_y.begin(), box_y.end())))),  // NOLINT
                   0,                                                                // NOLINT
                   height - 1);
  ...
  for (int i = 0; i < contour.size(); ++i) {
    rook_point[i] = cv::Point(int(box_x[i]) - xmin, int(box_y[i]) - ymin);  // NOLINT
  }
...
std::vector<std::vector<std::vector<int>>> BoxesFromBitmap(const cv::Mat pred, const cv::Mat bitmap,
  ...
  const int det_use_polygon_score = int(Config["det_use_polygon_score"]);  // NOLINT
  ...
  for (int i = 0; i < num_contours; i++) {
    ...
    for (int num_pt = 0; num_pt < 4; num_pt++) {
      std::vector<int> a{static_cast<int>(clamp(roundf(cliparray[num_pt][0] / float(width) *   // NOLINT
                                                       float(dest_width)),                     // NOLINT
                                                float(0),                                      // NOLINT
                                                float(dest_width))),                           // NOLINT
                         static_cast<int>(clamp(roundf(cliparray[num_pt][1] / float(height) *  // NOLINT
...
std::vector<std::vector<std::vector<int>>> FilterTagDetRes(std::vector<std::vector<std::vector<int>>> boxes,
  ...
  for (int n = 0; n < static_cast<int>(boxes.size()); n++) {
    ...
    for (int m = 0; m < static_cast<int>(boxes[0].size()); m++) {
      ...
      boxes[n][m][0] = static_cast<int>(std::min(std::max(boxes[n][m][0], 0), oriimg_w - 1));
      boxes[n][m][1] = static_cast<int>(std::min(std::max(boxes[n][m][1], 0), oriimg_h - 1));
    }
  }
  ...
  for (int n = 0; n < boxes.size(); n++) {
    ...
    rect_width =
        static_cast<int>(sqrt(pow(boxes[n][0][0] - boxes[n][1][0], 2) + pow(boxes[n][0][1] - boxes[n][1][1], 2)));
    rect_height =
        static_cast<int>(sqrt(pow(boxes[n][0][0] - boxes[n][3][0], 2) + pow(boxes[n][0][1] - boxes[n][3][1], 2)));
